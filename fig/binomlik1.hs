...
xlabels = [(0, "0")] ++ (map (f . (/ 10) . fromIntegral) [1..9]) ++ [(1, "1")]
  where f x = (x, showFFloat (Just 1) x "")

linearAxis :: (RealFloat a) => AxisFn a
linearAxis _ = ad { _axis_ticks = map (\(x,y)->(x,-y)) (_axis_ticks ad) }
...
setLayout = do
    ...
    layout_x_axis . laxis_generate .= linearAxis
...
-- outputFile = (FileOptions (800,500) PNG, "binomlik1_fig.png")
outputFile = (FileOptions (400,250) PDF, "binomlik1_fig.pdf")
-- outputFile = (FileOptions (400,250) PS, "binomlik1_fig.ps")
